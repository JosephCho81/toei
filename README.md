...
- 열: `차수 · 청구액 · 계산값 · 청구−계산 · 지급액 · 미수금 · 기일 · 상태`
- 「청구−계산」과 「미수금」은 **다른 열**이다. 합치면 35차 같은 건을 못 찾는다
...
- **방향은 하나뿐이다 — 에이원 → 토에이.** 그래서 같은 잔액이 토에이 시점에서는 「미수금」,
  에이원 시점에서는 「미지급금」이고, 「들어오다/나가다」도 시점에 따라 뒤집는다.
  KPI 는 `미수금/미지급금 · 기일 경과 · 90일 내 회수(지급) 예정 · 청구 예정` 네 장이다
