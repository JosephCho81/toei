...
## 화면 구성 — 정산 현황 vs 거래 목록

두 화면이 컬럼 6개(회차·제조사·LC개설·ETD·ETA·상태)를 겹쳐 보여주고 있어
2026-09-02 에 역할을 갈랐다. **같은 값을 양쪽에 다시 늘리지 말 것.**

| | 답하는 질문 | 컬럼 |
|---|---|---|
| **정산 현황** (`/dashboard`) | 돈이 언제 오가나, 무엇이 늦었나 | 회차 · 제조사 · 수입금액 · B/L · ETD · ETA · LC개설 · 중간정산 · 최종정산 · 상태 |
| **거래 목록** (`/transactions`) | 무엇을 얼마에 샀나, 어디가 틀렸나 | 오류 · 회차 · P/O No. · 제조사 · 품목(펼침) · ETA · 상태 |
