...
- **정렬은 차수 내림차순 하나뿐이다** (43차 위, 1차 맨 아래). 상태순으로 섞으면 「43차 다음이
  19차」가 되어 차수를 눈으로 좇을 수 없다 — 손댈 것은 상단 알림과 필터(전체/확인 필요/
  미완납/완납/청구 전)가 잡는다. 44개 차수를 접지 않고 전부 내보낸다
- 차수 하나가 `<tbody>` 하나다. 왼쪽 색막대(상태)와 굵은 「NN차」로 경계를 세운다.
  차수 표시는 `round_no` 로 한다 — `round_label` 은 연도가 어긋난 행이 있다(1차가 「26년 01차」)
- 분할 지급은 회차를 세로로 펴고, 바로 밑에 **「N회 합계 / 청구액 / 차액」 한 줄**을 둔다.
  이 줄이 이 화면의 결론이다. 합계는 저장값이 아니라 회차를 그 자리에서 더해 내고,
  뷰 집계(`paid_krw`)와 어긋나면 그 사실을 표시한다
- **기일 전 미지급은 빨갛게 「부족」이라 쓰지 않는다**(「미지급 — 기일 전」, 회색). 아직 낼 때가
  아닌 것을 연체와 같은 색으로 칠하면 진짜 연체가 묻힌다. 최종정산 미정산도 기일이 없어 주황이다
- 최종정산은 중간정산과 합치지 않고 차수 안에 한 줄로 나란히 둔다 — 합치면 어느 쪽이 안 맞는지 알 수 없다
- 지급 기록은 있는데 청구액이 없는 차수(25차)는 「청구 전」이 아니라 **「청구액 없음」**으로 부르고
  알림에 올린다 — 대사할 기준 자체가 없다는 뜻이다
