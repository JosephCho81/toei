...
| 메뉴 | 안에서 갈리는 것 | 가르는 곳 |
|---|---|---|
| 정산 대사 | 중간 · 최종 · 지체상금 | `CompareScreen` 탭 |
...
- 열: `선택 · 차수·P/O · 청구액 · 계산값 · 청구−계산 · 지급액 · 청구−지급 · 계산−지급 · 기일·실지급일 · 비고`
...
- **KPI 의 「미지급금」은 기일 경과분만 센다** (담당자 2026-09-05). 기일이 안 온 돈은
  못 받은 게 아니다 — 옆 칸 「기일 미도래」와 아래 **월별 결제 예정(넉 달)** 표가 받는다.
  빼기만 하고 안 보여주면 5억이 화면에서 사라진 것처럼 보인다
