...
-- ─────────────────────────────────────────
-- 추가 (2026-09-02, 위 스크립트 실행 이후)
-- ─────────────────────────────────────────
-- 26년 42차 통관환율이 비어 있었다.
-- 통관일 2026-08-25(유니패스 수입신고일) 기준 관세청 고시 USD 환율 = 1405.43
-- (2026-08-23 적용개시 주). 화면의 [관세청 대조] 버튼으로도 같은 값이 채워진다.
UPDATE transactions SET customs_exchange_rate = 1405.43
  WHERE round_no = 42 AND customs_exchange_rate IS NULL;
